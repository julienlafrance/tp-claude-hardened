...
# -----------------------------------------------------------------------------
# A LANCER SUR L'HOTE (corrin), PAS dans l'instance : `incus config device` est
# une commande cote hote. Cree/maj un proxy device Incus qui forwarde :
#     corrin:<SSH_HOST_PORT>  ->  <ip-du-conteneur-durci>:2222  (dropbear)
#
...
#   (corrin -> instance, puis instance -> conteneur) de maniere fiable.
#
...
SSH_HOST_PORT="${SSH_HOST_PORT:-2200}"   # 2222 est pris par Forgejo sur corrin
DEV="sshin"
...
echo "Bridge SSH OK : corrin:${SSH_HOST_PORT} -> ${CIP}:2222 (dropbear de ${NAME})"
echo "Depuis n'importe quel poste : ssh -p ${SSH_HOST_PORT} agent@<ip-corrin>  (cle id_rsa -> forced-command claude)"
